...
@section('content')
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2">Edit Data Paket Wisata</h1>
        </div>

        <form action="/admin/tour-packages/{{ $tour_package->slug }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-3">
                <label class="form-label">Gambar</label>
                <div id="image-container" class="d-flex gap-3 overflow-auto">
                    @foreach ($tour_package->tour_package_images as $image)
                        <div class="rounded border border-1 overflow-hidden preview-image-container">
                            ...
                            <input type="text" value="{{ $image->image }}" name="tour_package_image_in_storage[]" hidden>
                        </div>
                    @endforeach
                </div>
                @error('images.*')
            ...
            <div class="mb-3 col-lg-6">
                ...
                <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name"
                    value="{{ old('name', $tour_package->name) }}" required autocomplete="off">
                @error('name')
            ...
            <div class="mb-3 col-lg-6">
                ...
                @error('body')
                    <p class="text-danger">{{ $message }}</p>
                @enderror
                <input id="body" type="hidden" name="body" value="{{ old('body', $tour_package->body) }}">
