...
@section('content')
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        ...
        <form action="/admin/destinations/{{ $destination->slug }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <div id="image-container" class="d-flex gap-3 overflow-auto">
                    @foreach ($destination->destination_images as $image)
                        <div class="rounded border border-1 overflow-hidden preview-image-container">
                            ...
                            <input type="text" value="{{ $image->image }}" name="destination_image_in_storage[]" hidden>
                        </div>
                    @endforeach
                </div>
                @error('images.*')
            ...
            <div class="mb-3 col-lg-6">
                ...
                <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name"
                    value="{{ old('name', $destination->name) }}" required autocomplete="off">
                @error('name')
            ...
            <div class="mb-3 col-lg-6">
                ...
                @error('body')
                    <p class="text-danger">{{ $message }}</p>
                @enderror
                <input id="body" type="hidden" name="body" value="{{ old('body', $destination->body) }}">
